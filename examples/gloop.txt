...
void main(void) {
	...
	vec2 pos1 = vec2((1.0 + sin(iGlobalTime / 9.000)) / 2.0, (1.0 + sin(iGlobalTime / 7.100)) / 2.0) * ratio;
	vec2 pos2 = vec2((1.0 + sin(iGlobalTime / 8.900)) / 2.0, (1.0 + sin(iGlobalTime / 10.40)) / 2.0) * ratio;
	vec2 pos3 = vec2((1.0 + sin(iGlobalTime / 9.650)) / 2.0, (1.0 + sin(iGlobalTime / 91.50)) / 2.0) * ratio;
	...
	vec4 colour = vec4(0.8 * (1.0 - (fragCoord.y / iResolution.y)), 0.8 * (fragCoord.y / iResolution.y), 0.8, 1.0);
	if (dist > 0.0) {
